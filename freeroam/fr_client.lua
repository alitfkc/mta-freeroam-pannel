...
local function setElementPosition(element,x,y,z)
	if getElementData(localPlayer, "meta_meslek") == true then 
		showCursor(false)
		hideAllWindows()
...
---------------------------
local anim_engel_sure = 0

function applyAnimation(leaf)
	if getElementData(localPlayer,"eventsystem:paintballspawn") == true then  return false end
	if isPedInVehicle(localPlayer) then print("Araçta kullanamazsın") return end
	if (getTickCount()-anim_engel_sure) < 3000 then errMsg("Lütfen bekleyin!") return end
	...
	server.setPedAnimation(localPlayer, leaf.parent.name, leaf.name, true, false)
	anim_engel_sure = getTickCount()
end

function stopAnimation()
	if getElementData(localPlayer,"eventsystem:paintballspawn") == true then  return false end
	if getElementData(localPlayer,"işlemyapıor") == true then return end
	if getElementData(localPlayer, "meta_meslek") == true then return end 
	...
	controls = {
		{
			...
			height=290,
			columns={
				{text='Animasyonlar', attr='turkce'},
				{text='Anim ENG', attr='name'}
			},
			rows={xml='data/animations.xml', attrs={'name',"turkce"}},
...
-- Weapon window
---------------------------
local esya_engel_sure = 0
function addWeapon(leaf, amount)
	if (getTickCount()-esya_engel_sure) < 3000 then errMsg("Lütfen bekleyin!") return end
	if type(leaf) ~= 'table' then
...
local function retryTeleport(elem,x,y,z,isVehicle,distanceToGround)
	...
	if araba then
		local data = getElementData(araba, "Satilik") or false
		if data then
			return
		end
	end
	local hit, groundX, groundY, groundZ = processLineOfSight(x, y, 3000, x, y, -2999)
	if hit then
...
function setPlayerPosition(x, y, z, skipDeadCheck)
	...
	if not araba then
		if not isPedOnGround(localPlayer) then
			outputChatBox("Havadayken veya jetpack yüzünden ışınlanamaya bilirsin !", 255, 0, 0, true)
			return 
		end
	end
...
function updatePlayerBlips()
	...
	for elem,player in pairs(g_PlayerData) do
		...
		if yazi ~= (string.lower("Oyuncu Ara...")) then
			local playerName = player.name
			local playerName = (playerName:gsub("#%x%x%x%x%x%x", "")):lower()
		...
		if getElementData(elem,"blip_gizle_meta") then 
			visible = false
		end
		...
		--X Y Z KAPATMA
		-----------------------------------------------
			guiSetAlpha(getControl(wndSetPos,"x"),0)
			guiSetAlpha(getControl(wndSetPos,"y"),0)
			guiSetAlpha(getControl(wndSetPos,"z"),0)
		----------------------------------------------
	end
end
...
---------------------------
-- Interior window *******NOT:BU TELEPORT PANELİDİR.**********
---------------------------
local function setPositionAfterTeleport(x,y,z)
	setPlayerPosition(x,y,z+1)
	setCameraTarget(localPlayer)
	fadeCamera(true)
end

function setPlayerTeleport(x,y,z,i)
	--setCameraMatrix(x,y,z)
	--setCameraInterior(i)
	--server.setElementInterior(localPlayer, i)
	setTimer(setPositionAfterTeleport,1000,1,x,y,z)
end
...
function setTeleport(leaf)
	...
	if vehicle then
		--server.setElementInterior(vehicle, sectigiteleport.world)
		for i=0,getVehicleMaxPassengers(vehicle) do
			local player = getVehicleOccupant(vehicle, i)
			if player and player ~= localPlayer then
				--server.setElementInterior(player, sectigiteleport.world)
				--server.setCameraInterior(player, sectigiteleport.world)
			end
		end
	end
	--fadeCamera(false)
	setTimer(setPlayerTeleport,1000,1,sectigiteleport.posX, sectigiteleport.posY, sectigiteleport.posZ, sectigiteleport.world)
end
function teleportpaneliacma()
	if isWindowOpen(wndSetTeleport) then 
		closeWindow(wndSetTeleport)
	else
		panellerikapama()
		createWindow(wndSetTeleport)
	end
end
function secilenteleport(leaf)
	sectigiteleport=leaf
end
wndSetTeleport = {
	'wnd',
	text = 'Teleport',
	...
	controls = {
		{
			...
			height=180,
			columns={
				{text='        Adı', attr='adi',width=0.65},
				{text='   Bölge', attr='bolge',width=0.25}
			},
			rows={xml='data/teleports.xml', attrs={'adi','bolge', 'posX', 'posY', 'posZ', 'world'}},
			onitemclick=secilenteleport,
			onitemdoubleclick=setTeleport,
			DoubleClickSpamProtected=true,
		},
		{'btn', id='SEÇ',onclick=setTeleport,width=210,height=30}
	}
}
...
function setPlayerInterior(x,y,z,i)
	...
	--setCameraInterior(i)
	--server.setElementInterior(localPlayer, i)
	setTimer(setPositionAfterInterior,250,1,x,y,z+1)
end
...
function setInterior(leaf)
	...
	--fadeCamera(false)
	setTimer(setPlayerInterior,250,1,leaf.posX, leaf.posY, leaf.posZ, leaf.world)
...
---------------------------
local arac_engel_sure = 0
function createSelectedVehicle(leaf)
	if (getTickCount()-arac_engel_sure) < 10000 then return errMsg("Lütfen biraz bekleyin!") end
	if not leaf then
...
function repairVehicle()
	...
	if isPlayerInEvent() then return end
	local vehicle = getPedOccupiedVehicle(localPlayer)
	if spamEngel == false then return errMsg("Her 5 saniyede bir aracını tamir edebilirsin.") end
	if vehicle then
		local benzin = getElementData(vehicle, 'fuel')
		if benzin and benzin < 2 then
			setElementData(vehicle, 'fuel', 1)
		end
		...
		--spamEngelF()
		if getPedOccupiedVehicleSeat(localPlayer) ~= 0 then return errMsg("Lütfen sürücü koltuğuna geçin.") end
		server.fixVehicle(vehicle)
	    triggerEvent("zirh_ver",localPlayer,vehicle)
	end
end
...
function light_onoff()
	...
	if getVehicleOverrideLights(vehicle) == 2 then 
		...
		guiSetText(getControl(wndMain,"lights"),"Işıkları Aç")
	else
		server.setVehicleOverrideLights(vehicle, 2)
		guiSetText(getControl(wndMain,"lights"),"Işıkları Kapat")
	end
end
...
function openColorPicker()
	...
	if (editingVehicle) then
		local data = getElementData(getPedOccupiedVehicle(localPlayer), "Satilik") or false
		if data then
			return
		end
		if getPedOccupiedVehicleSeat(localPlayer) ~= 0 then return errMsg("Lütfen sürücü koltuğuna geçin.") end
...
function onlastikizi() --meta
	local state = guiCheckBoxGetSelected(getControl(wndSetting, 'lastikizi'))
	triggerEvent("lastikizi_ac",localPlayer,state)
end
function onkarmodu() --meta
	print("hop")
	triggerEvent("karmodu",localPlayer)
end
...
function toggleWarping()
	...
	outputChatBox("İnsanların sana ışınlanması "..(state and "kapatıldı." or "açıldı."),(state and 255 or 0) ,(state and 0 or 255),0)
...
function mainWndShow()
	if not getPedOccupiedVehicle(localPlayer) then
		hideControls(wndMain, 'repair', 'flip', 'upgrades', 'color',"lights")
	end
...
function onEnterVehicle(vehicle,seat)
	if source == localPlayer then
		setControlText(wndMain, 'curvehicle', getVehicleName(vehicle))
		showControls(wndMain, 'repair', 'flip', 'upgrades', 'color', 'lights')
		if getVehicleOverrideLights(vehicle) == 2 then 
			guiSetText(getControl(wndMain,"lights"),"Işıkları Kapat")
		else
			guiSetText(getControl(wndMain,"lights"),"Işıkları Aç")
...
wndAyarlar = {
	...
	y = 0.25,
	controls = {
		{'chk', id='ackapa1', text='Detaylar Aç/Kapat', onclick=detaylarackapa},
		{'chk', id='ackapa2', text='Deniz Aç/Kapat', onclick=hddenizackapa},
		{'chk', id='ackapa6', text='Bulut Aç/Kapat', onclick=gokyuzuackapa},		
		{'chk', id='ackapa3', text='Araba Boyası Aç/Kapat', onclick=arababoyaackapa},
		{'chk', id='ackapa4', text='Kontrast Aç/Kapat', onclick=hdr_ackapa},
		{'chk', id='ackapa5', text='Bloom Aç/Kapat', onclick=bloomackapa},
		{'chk', id='ackapa7', text='Chat Aç/Kapat', onclick=ChatAcKapat},
		--{'chk', id='ackapa8', text='Öldürme mesaj Aç/Kapat', onclick=killMesaj},
		{'chk', id='disablewarp', text='Işınlanma', onclick=toggleWarping},
		{'chk', id='lastikizi', text='Lastik izi Aç/Kapat', onclick=onlastikizi}, --meta
		{'chk', id='kar', text='Kar Modu', onclick=onkarmodu}, --meta
		--{'chk', id='ackapa9', text='Araç Hayalet Aç/Kapat', onclick=hayaletMod},
	--	{'br'},		
		--{'lbl', id='labell', text='Nişangahlar'},	
	--	{'br'},				
		--{'rad', id='nisangah1', text='Nişangah 1', onclick=Nisangah1Aktif},
		--{'rad', id='nisangah2', text='Nişangah 2', onclick=Nisangah2Aktif},
		--{'rad', id='nisangah3', text='Nişangah 3', onclick=Nisangah3Aktif},
		--{'rad', id='nisangah4', text='Nişangah 4', onclick=Nisangah4Aktif},
		--{'rad', id='nisangah5', text='Nişangah 5', onclick=Nisangah5Aktif},		
		--{'rad', id='nisangah6', text='Nişangah 6', onclick=Nisangah6Aktif},		
		--{'btn', id='Nişangahı Sıfırla', onclick=NisangahPasif},
		{'btn', id='Kapat', closeswindow=true,width = 230,x = 10}
	}
}
...
wndOyunlar = {
    ...
    y=0.35,
    controls = {
        {'btn', id='Squid Game',onclick=sq_evente_katil,width=210,height=30},
    }
} 
------------------------------
...
wndMain = {
	...
	controls = {
		{'br'},
		{'btn', id='Kendini Öldür', onclick=killLocalPlayer, width = 125,x = 10},
		{'btn', id='Ayarlar', window=wndAyarlar, width = 125,x = 140},
		{'btn', id='Animasyonlar', window=wndAnim, width = 125,x = 140},
		{'btn',id='havadurum',text='Hava Durumu',window=wndWeather, width = 125,x = 10},
		{'btn', id='yeniisinlanma', text='Görevler', window=wndSetInterior, width=125, x=10},
		{'btn',id='skin',text='Karakterler',window=wndSkin, width = 125,x = 140},
		--{'btn', id='gorevsistem', text='Mekanlar', onclick=mekanlarpaneli, width=255, x=10},
		--{'chk', id='yapisma', text='Araç Yapışma', onclick=yapisma_ackapa, width = 95,x = 140},
		{'btn', id='teleport', text='Mekanlar', window=wndSetTeleport, width=255, x=10},	
		{'btn', id='setpos', text='Harita', window=wndSetPos, width=255, x=10},	
		{'btn', id='createvehicle', text='Araç Oluştur', window=wndCreateVehicle, width = 255,x = 10},
		--{'btn', id='airpanels', text='Araç Air', onclick=airpanels, width = 70,x = 100},
		--{'btn', id='AracKontroller', text='Araç Kontrol', onclick=enableVehicleControl, width = 80,x = 185},
		{'btn', id='repair',text="Tamir", onclick=repairVehicle, width = 125,x = 10},
		{'btn', id='flip',text="Döndür", onclick=flipVehicle, width = 125,x = 140},
		--{'btn', id='Color',text="Renk", onclick=openColorPicker, width = 125,x = 140},
		--{'btn', id='upgrades',text="Eklentiler", window=wndUpgrades, width = 125,x = 10},
		{'btn', id='lights',text="Işıkları Aç",lights,onclick=light_onoff, width = 255,x = 10,},
		--{'chk', id='lightson', text='Farı [Aç]', onclick=forceLightsOn, width = 105,x = 10},
		--{'chk', id='lightsoff', text='Farı [Kapat]', onclick=forceLightsOff, width = 105,x = 140},
	},
