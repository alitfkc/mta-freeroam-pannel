...
float4 PixelShaderFunction(PSInput PS) : COLOR0
{
    ...
    float microflakePerturbation = 0.001;
    float brightnessFactor = 35.00; // the default value is 0.10
    float normalPerturbation = 1110.00;
    float microflakePerturbationA = 0.05;
    ...
    vFlakesNormal = 1 * vFlakesNormal - 1.0;
    ...
    float4 paintColor = fFresnel1 * paintColor0 +
        ...
        pow( fFresnel2, 90 ) * flakeLayerColor;

    // Combine result of environment map reflection with the paint color:
    float fEnvContribution = 1.1 - 0.5 * fNdotV;
    ...
    float4 Color = 1;
    Color = finalColor / 1 + PS.Diffuse * 0.9;
    Color += finalColor * PS.Diffuse * 10.9;
  Color *= 0.009; // If you don't want to see the dirt texture just delete that line
    Color.a = 0.599;
